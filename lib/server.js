...
p.init = function () {
    ...
    this.server = http.createServer(function (req, res) {
        ...
        var commonJSsupported = /commonjs=false/i.test(query) ? false : true;
        ...
        fs.readFile(responseFile, function (err, data) {
            if (err) {
				...
				} else if ((isJs(responseFile) || isJsx(responseFile)) && commonJSsupported){ // 处理 js/jsx 里的 CommonJS 语法
					res.writeHead(200,{
						'Content-Type': contentType + '; charset=utf-8'
					});
